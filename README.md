...
> **Scoreboard — checked scoped results: ω-model: 10 (kernelChecked); all-model: 1
...
All ten facts are kernel-checked over **every Turing ideal** against the
...
| `locallyFinitePerfectMatchingKonigOmega` | locally finite perfect matchingω ⇔ full finitely-branching Kőnigω | Hirst's symmetric marriage theorem (Thm 3.1) in its property-shaped presentation: one bare edge set, per-side local finiteness as an existential property, cardinality-form H_sym — a new concept, deliberately separate from the countableHall family and the enumerated two-regular presentation class; jump closure stays one certified hop away, reached by computed closure only |
