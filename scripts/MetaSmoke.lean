...
info: concepts (8):
  ...
  reverse-mathlib:finitelyBranchingKonig — Full finitely-branching Kőnig's lemma: every infinite finitely branching tree has an infinite path — the branching bound is a property of the tree (at each position some bound on the entries exists), never supplied data
