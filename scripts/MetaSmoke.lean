...
info: concepts (5):
  ...
  reverse-mathlib:disjointRangeSeparation — Disjoint-range separation as a conceptual family: separating sets for pairs of injections with disjoint ranges (Hirst Thm 1.2 (ii) / Simpson's Σ⁰₁-separation circle). The registered presentation is the exact injection-graph form; formula-coded Σ⁰₁ separation, arbitrary-function, and enumeration presentations join only once their adapters are proved
    statement: Disjoint-range separation: for every pair of injections with disjoint ranges there is a separating set that contains every value of the first injection and no value of the second
  ...
  reverse-mathlib:explicitFiniteInverseLimitCompactness — Explicit finite inverse-limit compactness as a conceptual family: sequential systems of explicitly enumerated finite fibers with adjacent bonding maps
    statement: Explicit finite inverse-limit compactness: every sequential inverse system of nonempty, explicitly enumerated finite fibers with bonding maps between adjacent levels has a section — a choice of one point per level respecting every bonding map
  ...
  reverse-mathlib:rca0Core — The second-order core of RCA₀ at a fixed first-order part: nonemptiness, downward Δ⁰₁ (Turing) closure, and closure under recursive join — the Turing-ideal closure conditions. A base-capability concept: it names what the base theory's ω-models satisfy by definition, so ω-scope separations have an honest typed lhs; it is not a theorem-strength concept and carries no external crosswalk
    statement: Not a mathematical principle but a base capability: a second-order part is an RCA₀ core when it is nonempty, closed downward under Turing reducibility, and closed under recursive join — exactly what the base theory's ω-models satisfy by definition
  ...
  reverse-mathlib:wkl — Weak Kőnig's lemma as a conceptual family: binary-tree formulations across semantic layers (ambient / ω-model / second-order syntax), plus the explicitly bounded ω-model formulation, joined through the kernel-checked presentation equivalence boundedKonigWklOmega. Merely finitely branching (full Kőnig) is the ACA-level principle and belongs to a separate concept, not under the rmzoo:WKL alias
    statement: Weak Kőnig's lemma: every infinite binary tree — a prefix-closed set of finite bit sequences with a node at every level — has an infinite path
...
info: facts (10):
  ...
  disjointRangeSeparationWklOmega [equivalence | theory rca0 omegaModels] disjointRangeSeparation.injectionGraphs.turingIdealOmega <=> wkl.binaryTree.turingIdealOmega — recorded, no evidence linked
    note: The sixth production ω fact: the injection-graph disjoint-range separation and binary-tree WKL variants are equivalent at the Turing-ideal ω layer. Both directions were frozen before registration — the interface was deliberately built bridge-local during the matching tranche so this fact would arrive with worked artifacts on both sides. The presentation is exactly injection graphs: no formula-coded Σ⁰₁ adapter is proved, and no generic Σ⁰₁-separation claim is made
